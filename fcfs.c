...
void main()
{
    ...
    printf("\nProcess Name\tArrival Time\tBurst Time\tStart Time\tFinish Time\tWaiting Time\tTurn Around Time\n");
    for (i = 0; i < n; i++)
    {
        printf("%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n", process_num[i], arrival_time[i], burst_time[i], start[i], finish[i], wait_time[i], tat[i]);
